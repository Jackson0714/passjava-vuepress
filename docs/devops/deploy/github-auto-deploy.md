...
我有一个个人的博客网站 passjava.cn，最新的文章都会首发到该网站，如下图所示：

![](http://cdn.passjava.cn/uPic/image-20260518201751009Ts9PO8MaMi3G.png)

尝试过多种更新文章内容到网站的方式，都有较大的缺点：

- **方式一**：`Jenkins` 打包部署到远程服务器。缺点：需要多点几步。

- **方式二**：本地`命令行`上传到远程服务器。缺点：还需要多执行一步。

- **方式三**：调用 `MCP` 服务上传到远程服务器。缺点：mcp 服务不稳定，有时候成功， 有时候不成功。可以看我之前写的这篇用 MCP 部署博客的文章：《巧用智能体+100行代码的MCP服务，打造一个简易版“智能化运维”平台》

- **方式四**：登录远程服务器，获取github 的最新代码，执行打包命令。缺点：依赖 node 环境，下载 npm 依赖包总是失败，编译报错。
