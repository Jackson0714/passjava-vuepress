---
title: 06｜底层架构（上）：MCP 协议层（Protocol layer）详解
date: 2025-06-24
---
...
![image-20250624090804820](http://cdn.passjava.cn/uPic/image-20250624090804820wzr8DowP9i0D.png)
...
作者回复: 再次感谢悟空兄弟！思考题的回答非常棒。

疑问1. ReceiveResultT 看这里 https://github.com/modelcontextprotocol/python-sdk/blob/main/src/mcp/shared/session.py SendRequestT = TypeVar("SendRequestT", ClientRequest, ServerRequest) SendResultT = TypeVar("SendResultT", ClientResult, ServerResult) SendNotificationT = TypeVar("SendNotificationT", ClientNotification, ServerNotification) ReceiveRequestT = TypeVar("ReceiveRequestT", ClientRequest, ServerRequest) ReceiveResultT = TypeVar("ReceiveResultT", bound=BaseModel) ReceiveNotificationT = TypeVar("ReceiveNotificationT", ClientNotification, ServerNotification)

疑问2：message_handler 笔误？应该为 logging_callback？ 是这样的：在/mcp/client/session.py这个程序中。 logging_callback 处理特定的日志通知 message_handler 处理所有类型的传入消息，提供统一的上层监控和处理能力 因此，说“其他非同步钩子捕获的消息和未在钩子中处理的所有请求、通知，都会统一委托给用户在构造时提供的 message_handler”，应该是OK的。

更清晰的说法是： - 对于通知，`_received_notification(notification)` 目前只对日志消息（LoggingMessageNotification）作出处理，调用专门的 `logging_callback`。 - 对于请求，`_received_request(responder)` 处理特定的请求类型（如 CreateMessageRequest、ListRootsRequest、PingRequest），调用相应的专门回调函数。 - 所有其他类型的通知和请求（包括在专门钩子中未处理的），以及异常，都会统一委托给用户在构造时提供的 `message_handler`，以便在上层进行监控、界面更新或其他自定义处理。 大家看看这样讲清晰了不？
...
![img](http://cdn.passjava.cn/uPic/ae42e50c08b90095cc64c35c422d41b8hqNJe4.jpg)
