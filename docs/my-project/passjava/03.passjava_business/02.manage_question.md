...
![mark](http://cdn.passjava.cn/blog/20200426/dWB1QSPspqJN.png?imageslim)
...
```vue
dataRule: { displayOrder: [ { validator: (rule, value, callback) => { if (value
== "") { callback(new Error("排序字段必须填写")); } else if
(!Number.isInteger(value) || value<0) { callback(new
Error("排序必须是一个大于等于0的整数")); } else { callback(); } }, trigger:
"blur" } ] }
```
...
![后端字段校验](http://cdn.passjava.cn/blog/20200430/D4xbDjs5c0z7.PNG)
