---
title: 48 张图 | 手摸手教你微服务的性能监控、压测和调优
category: 项目实战
tag:
  - passjava面试刷题项目
---

![本文主要内容](http://cdn.passjava.cn/uPic/0ead02dc0f4529736802d962985ef0cd6qmiqM.png)
...
![](http://cdn.passjava.cn/uPic/8b33973211e6af588cea4d66427864edr1Zsis.png)

- 运行 JMeter 程序

打开批处理文件：\apache-jmeter-5.3\bin\jmeter.bat

![](http://cdn.passjava.cn/uPic/9394eb8e9e21b096dd718aece354d155gFQnqd.png)

- 添加线程组

![添加线程组](http://cdn.passjava.cn/uPic/fcb5e7b525cf592cbbe2b061cced69144Oin5i.png)

- 1s 内启动 200 个线程，循环次数 100 次。2w 个请求。

![](http://cdn.passjava.cn/uPic/343068fbe24af2f51949faa088efa800WHxrHS.png)

- 测试 HTTP 请求

![](http://cdn.passjava.cn/uPic/2fb320d0fefdfe8c72a9d629b69efc15Lbd6Ui.png)
...
![](http://cdn.passjava.cn/uPic/968761e1e6249c12cfca25af6f9e875fvmIOpX.png)

- 添加察看结果树、汇总报告和聚合报告

![](http://cdn.passjava.cn/uPic/fa8614816e1e6e04df6730796917c895Stxdbr.png)

- 开始压力测试

  点击播放按钮就开始启动了。注意启动之前需要先设置线程组的参数配置和 HTTP 请求的配置。

![](http://cdn.passjava.cn/uPic/244d6d62e872037e7ee3de8bb6d20c47VS2ueu.png)

- 查看每个请求结果

![](http://cdn.passjava.cn/uPic/dad8b755fee01265db1ea22e57db3fa2fC6nKF.png)
...
![](http://cdn.passjava.cn/uPic/e630f82324d4be550eb4c4dc70b06f174zNGJF.png)
...
![](http://cdn.passjava.cn/uPic/5e5089d18ba60620c051fc945f01485cJMjq6E.png)

- 查看汇总图

查看汇总图时，需要先勾选想要查看的信息

![](http://cdn.passjava.cn/uPic/501784df9c1aba0599899c5a0535e3depJArrc.png)

然后查看图形汇总：

![](http://cdn.passjava.cn/uPic/cf6d85851b1984343598910d3e0a41c01FM5V6.png)

可以看到勾选的几列在图表中是用不同颜色表示的，比如绿色的柱状条就是 90 % 百分位。

我们来测试下 佳必过的管理后台的性能，吞吐量接近 2000/s。

![](http://cdn.passjava.cn/uPic/308de4ba0b94a7f9667780c2e7797e3ekYX22j.png)
...
![命令行启动 jconsole](http://cdn.passjava.cn/uPic/4d2f4df5b6fea0d0625bf65e864898176sR3KM.png)

### 选择监控哪个应用

然后选择 passjava 项目的 question 服务。

![选择 passjava-question 微服务](http://cdn.passjava.cn/uPic/8475056b049e5d8a124472bd7255620byjMWvW.png)

对应的就是下面这个微服务： passjava-question

![对应 passjava-question 微服务](http://cdn.passjava.cn/uPic/b6f719d9d74b2294bd2d36333a834f09elPrmR.png)

### 概览

从监控界面上有 6 个菜单，首先看到的是概览功能，上面有堆内存使用量、线程数、类的使用情况、CPU 占用率，都是用趋势图来表示的，能很方便的看出当前性能的概览。注意：这些监控都是实时的。

![概览](http://cdn.passjava.cn/uPic/d96c05f6fa49da0dcdf2e27ae9a929f8uZpBE8.png)

### 内存

下面是内存的使用情况，可以从下图中看到有个下拉框，里面可以选择不同的内存维度，然后下面的图标和柱状图也会跟着选择的维度而展示不同。

![](http://cdn.passjava.cn/uPic/3779a386c80c7ff619a7d36731e87a70R9SIXh.png)

### 内存

下面是线程的使用情况，可以看到线程峰值和活动线程的总数量，目前看到的峰值是59，活动线程数是 57。下半部分可以看到具体是哪些线程，以及线程的堆栈信息，非常详细。

![线程使用情况](http://cdn.passjava.cn/uPic/41add8f3062004f5bbfa0a9f85740f32wp6v5r.png)

### 类

下面是类的加载和卸载情况，已加载类总数是 10679，而已卸载的类是 1 个，所以当前已加装当前类的总数是 10679 - 1 = 10678 个。

![类的加载和卸载情况](http://cdn.passjava.cn/uPic/f3a8d1b410a99abc32f473b41b7df05bMKOw64.png)

### VM 概要

我们再来看下VM（虚拟机）的情况。如下图所示，可以看到虚拟机情况，线程、类、堆的概要信息，以及 VM 的参数，是不是很方便呀~

![VM 概要](http://cdn.passjava.cn/uPic/f3ecca538e4d0d613c76d9770e820e25j9CJus.png)
...
![MBean 信息](http://cdn.passjava.cn/uPic/ac523e4d74a703c41ffba7b2d2d57bee1pnV2u.png)
...
![启动jvisualvm](http://cdn.passjava.cn/uPic/a187ba33454ebfc8ea6985805ea578b16BsEYQ.png)

### 监视

监视 CPU、堆、类、线程的情况。整体显示的效果比 jconsole 更美观。

![监视](http://cdn.passjava.cn/uPic/06dde70962d0b051d50d05e88ac03854UYpWc8.png)
...
![](http://cdn.passjava.cn/uPic/6c6041a687bfb735e305408bc0a8caaaI7QyJW.png)

### 抽样器

另外我们也可以抽样器对 CPU 或内存进行抽样。如下图所示，对内存进行抽样。

![抽样](http://cdn.passjava.cn/uPic/5c793a79a47a6a0805a9f7127092ba88MBHBlB.png)
...
![](http://cdn.passjava.cn/uPic/00f53ce5f4c81d28222c0fcf6c2c8a53sLFOQV.png)

安装完成后，就可以看到

![Visual GC 插件](http://cdn.passjava.cn/uPic/f2ef239cd7fb7d75b82a77dfa9bc51fa91JiAv.png)

下图是实时监控垃圾回收的情况。

![Visual GC](http://cdn.passjava.cn/uPic/0c5ad825560521e7abfcfc2a2bc0b2e06CmdGn.png)
...
![](http://cdn.passjava.cn/uPic/25b921d7571d1060d55eb429206f4056lohLGM.png)
...
![](http://cdn.passjava.cn/uPic/3fb46db6fbb595828cc130e65f4695beI5eXCm.png)

配置每秒发送 200 个请求，一直循环执行，直到手动停止压测。如下图所示：

![](http://cdn.passjava.cn/uPic/a3ce15fcd12e71df4b4cb80ebc5b3a35XNnwzK.png)
...
![](http://cdn.passjava.cn/uPic/1c71eae0ac70ff4dd1e8e76807bd1a77UyoZvl.png)

我们再来看看垃圾回收的情况，Eden 区垃圾回收用时 2.7 s，用时太长了吧，看看这里怎么能优化下。

通常的优化方向是增大新生代堆内存配置。

![](http://cdn.passjava.cn/uPic/6b5ff65b79ed67041846944e698deb3fVnAacE.png)
...
![测试方法](http://cdn.passjava.cn/uPic/29cdc005a8e928b7c7ce7b7452568d9fddwMta.png)

有两种方式测试这个 api 是否添加正确。

第一种用 postman 测试下这个请求是否能正确响应，返回 “test” 则表示响应正确。

![test api 是否能正确响应](http://cdn.passjava.cn/uPic/fde60206467bfc8951ad6435e6b14a5c80Pmr7.png)

第二种通过浏览器进行测试。浏览器地址栏输入以下链接后，回车，看下浏览器窗口是否显示 “test”，是则表示响应正确。

然后我们需要用 Jmeter 压测工具来测试这个微服务下的 api 的性能究竟如何。

![单独压测微服务的 api的结果](http://cdn.passjava.cn/uPic/16f5eae40b8988c2730ddf5c97e2dedfqvEY54.png)
...
![网关+微服务的压力测试结果](http://cdn.passjava.cn/uPic/792c42af8583125e1e5b84013ab14469DKun7a.png)
...
![横向对比](http://cdn.passjava.cn/uPic/43c2e652bee5ebd5ee5a509ec76a8644t69YIH.png)
...
![查询问题列表的 api](http://cdn.passjava.cn/uPic/899a7637f378aa37de27f72d50ebe45cS2gvcjzlgsfN.png)

我们加些测试代码：统计查询数据库的耗时。如下所示：

![耗时统计](http://cdn.passjava.cn/uPic/18e11d292f3589bc68f0923f64af96cc8gmPIFug3IKO.png)

然后重启 passjava-question 服务，再次测试这个 api，耗时 43 ms

![](http://cdn.passjava.cn/uPic/f4f49bbdbed6360495dbc33438cf6051FHT6Jd.png)

怎么对查询进行优化呢？很容易想到加索引，我们来试下加在 question 表加索引后的效果。给 type 字段加上普通索引，如下图所示：

![添加索引](http://cdn.passjava.cn/uPic/9bc76b5870c0a3b8480f03ec61d72480KfM8rOk2MY70.png)

我们再来看下加了索引后的耗时情况：耗时 18 ms，确实比之前的 43 ms 快了很多。

![加了索引后的情况](http://cdn.passjava.cn/uPic/d9f8d38ae103359594134e47da6dd9cd5daAok.png)
...
![32M Eden 区频繁进行垃圾回收](http://cdn.passjava.cn/uPic/7704b030fba57010fbeb91334621e14aYCDyEi.png)
...
![](http://cdn.passjava.cn/uPic/86cbe05e08def8bbc92d30c9cf1c86e20cf7By.png)
