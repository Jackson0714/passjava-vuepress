...
![主要内容](http://cdn.passjava.cn/blog/20210407/7NH7YHjjDMQW.png?imageslim)
...
![支持多语言](http://cdn.passjava.cn/blog/20210324/fg7pE47tRdC6.png?imageslim)
...
![Java API 已经废弃了](http://cdn.passjava.cn/blog/20210324/qPTAYbNhHzwA.png?imageslim)

所以我们只能用 Java REST Client 了。而它又分成两种：高级和低级的。高级包含更多的功能，如果把高级比作MyBatis的话，那么低级就相当于JDBC。所以我们用高级的 Client。

![高级和低级 Client](http://cdn.passjava.cn/blog/20210330/Se52lN1s5wME.png?imageslim)
...
![选择 Spring Initializr](http://cdn.passjava.cn/blog/20201008/XKlqVxQ83s8v.png?imageslim)

第二步：填写模块信息，然后点击 Next。

![passjava-search 服务模块](http://cdn.passjava.cn/blog/20201008/OLby7KmG76Uh.png?imageslim)

第三步：选择 Web->Spring Web 依赖，然后点击 Next。

![mark](http://cdn.passjava.cn/blog/20210330/4zSqqaqvEdHw.png?imageslim)
...
![Rest Client 官方文档](http://cdn.passjava.cn/blog/20201008/IIcFSMBBHgkc.png?imageslim)
...
![设置版本为 7.4.2](http://cdn.passjava.cn/blog/20201008/bboz9aCt91NE.png?imageslim)
...
![ES 测试结果](http://cdn.passjava.cn/blog/20201008/4gpzgYrKQ5vL.png?imageslim)
...
![控制台输出结果](http://cdn.passjava.cn/blog/20201014/7FpsoV1fzT2w.png?imageslim)
...
![查询 users 索引结果](http://cdn.passjava.cn/blog/20201014/PENUrSXiHBlR.png?imageslim)
...
![查询复杂语句示例](http://cdn.passjava.cn/blog/20201015/RJtuH9CpUa9H.png?imageslim)

将打印出来的检索参数复制出来，然后放到 JSON 格式化工具中格式化一下，再粘贴到 ES 控制台执行，发现执行结果是正确的。

![打印出检索参数](http://cdn.passjava.cn/blog/20201015/IRiUeDIFjRAL.png?imageslim)

用在线工具格式化 JSON 字符串，结果如下所示：

<img src="http://cdn.passjava.cn/blog/20201015/Q6OCLg0bwTMo.png?imageslim" alt="格式化 JSON 字符串" style="zoom:50%;" />

然后我们去掉其中的一些默认参数，最后简化后的检索参数放到 Kibana 中执行。

Kibana Dev Tools 控制台中执行检索语句如下图所示，检索结果如下图所示：

![控制台中执行检索语句](http://cdn.passjava.cn/blog/20201015/2lMEF72wOc3b.png?imageslim)
...
![bankMember](http://cdn.passjava.cn/blog/20210331/fIebfHqc6JrA.png?imageslim)
...
![ES 返回的年龄分布信息](http://cdn.passjava.cn/blog/20210331/SjpKN4iqeq9L.png?imageslim)
...
![打印结果：用户年龄分布](http://cdn.passjava.cn/blog/20210331/rEylrPkHFMLC.png?imageslim)

#### 1.7.4 获取平均薪资聚合信息

现在来看看平均薪资如何按照所需的格式返回，ES 返回的结果如下图所示，我们需要获取 balanceAvg 字段的 value 值。

![ES 返回的平均薪资信息](http://cdn.passjava.cn/blog/20210331/maaMHKEKI8HV.png?imageslim)
...
![打印结果：平均薪资](http://cdn.passjava.cn/blog/20210331/SpivcKRO72ch.png?imageslim)
...
![创建 question 索引](http://cdn.passjava.cn/blog/20210401/DXVL2Ienzfp3.png?imageslim)
...
![查看 ES 中所有的索引](http://cdn.passjava.cn/blog/20210406/grzhbrDtFk36.png?imageslim)
...
![mark](http://cdn.passjava.cn/blog/20210406/2uTN9VUrDGr4.png?imageslim)
...
![用 model 来组装数据](http://cdn.passjava.cn/blog/20210406/bEn1XFkKkvGK.png?imageslim)

### 3.6 保存数据到 ES

我在 passjava-search 微服务中写了一个保存题目的 api 用来保存数据到 ES 中。

![保存数据到 ES](http://cdn.passjava.cn/blog/20210406/LarcBp8VLogp.png?imageslim)
...
![mark](http://cdn.passjava.cn/blog/20210406/hTTJ5FNo3FrE.png?imageslim)
...
![组装查询参数](http://cdn.passjava.cn/blog/20210407/wv4TCH9Putzv.png?imageslim)
...
![格式化 ES 返回结果](http://cdn.passjava.cn/blog/20210407/3ULeACjuDij1.png?imageslim)
...
![测试匹配 title](http://cdn.passjava.cn/blog/20210407/XES3SN0VYKS1.png?imageslim)

#### 4.5.2 实验二：测试 answer 匹配

我们现在想要验证 answer 字段是否能匹配到，传的请求参数 keyword = 测试答案，匹配到了 title = 测试答案的数据，且只有一条，说明查询成功。如下图所示：

![测试匹配 answer](http://cdn.passjava.cn/blog/20210407/aJL2IgNtXN7o.png?imageslim)
...
![测试 id 匹配](http://cdn.passjava.cn/blog/20210407/COELzyGcfWk2.png?imageslim)
