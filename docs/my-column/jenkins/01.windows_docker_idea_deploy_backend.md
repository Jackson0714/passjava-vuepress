---
title: Windows + Docker + IDEA 部署后端项目
date: 2024-08-18
---

# Windows + Docker + IDEA 部署后端项目
...
![目录](http://cdn.passjava.cn/uPic/image-20230302181650720S4EhS9.png)

## 一、背景

![Docker打包部署方案](http://cdn.passjava.cn/uPic/image-202303021725072417f414I.png)
...
![Windows+虚拟机+Docker](http://cdn.passjava.cn/uPic/image-20230302105009007u8OfqP.png)

Windows 自带的 Hyper-V 工具就是一款管理和创建虚拟机的工具，要求：Windows 10 企业版、专业版或教育版。启用 Hyper-V 如下所示：

![启用 Hyper-V](http://cdn.passjava.cn/uPic/1513668234-6433-20171206211858191-1177002365QKbXFd.png)
...
![](http://cdn.passjava.cn/uPic/image-20230302110619135ycFrd8.png)
...
![](http://cdn.passjava.cn/uPic/image-20230302154643799UP13uv.png)
...
![](http://cdn.passjava.cn/uPic/image-20230302144510906L3c5kv.png)
...
![](http://cdn.passjava.cn/uPic/image-202303021525560011y8zV7.png)
...
![](http://cdn.passjava.cn/uPic/image-20230302152330666KAN2MN.png)
...
![](http://cdn.passjava.cn/uPic/image-202303021556306206SrGOE.png)

咦？居然访问不通，这是为啥？？

我们来排查下。先看下容器日志，执行命令 docker logs <容器id>，可以看到 Java 应用程序正常启动，也没有报错日志。

![](http://cdn.passjava.cn/uPic/image-20230302155743185mWYnbh.png)那说明可能是网络问题，比如容器的端口和我们本机的端口没有映射。果不其然。这里端口是否没有映射的，如下图所示。

![](http://cdn.passjava.cn/uPic/image-20230302155925256xh9s5A.png)
...
![](http://cdn.passjava.cn/uPic/image-20230302160436562XFCZdT.png)

话不多说，直接运行起来看下。

![](http://cdn.passjava.cn/uPic/image-202303021621574051rLtn7.png)

再来看下端口映射，这次就映射好了，完美！

![](http://cdn.passjava.cn/uPic/image-20230302162226372Gac1rM.png)
...
![](http://cdn.passjava.cn/uPic/image-20230302162357432EdnCo9.png)
...
![](http://cdn.passjava.cn/uPic/image-20230302164243271y0X4yE.png)
...
![将镜像拷贝到服务器](http://cdn.passjava.cn/uPic/image-20230302163254068eicgCs.png)
...
![passjava-demo镜像包](http://cdn.passjava.cn/uPic/image-202303021645335438GBfEi.png)
...
![启动报错，找不到 jar 包](http://cdn.passjava.cn/uPic/image-20230302164857810tkIdp7.png)

哦，想起来了，挂载上面的 apps 目录后，容器会从容器里面的 apps 目录找 JAR 包，发现没有这个 jar 包，当然报错了呀！

![运行命令](http://cdn.passjava.cn/uPic/image-202303021650182487W9QFG.png)

而这个 apps 目录正好是和宿主服务器的 /nfs-data/service/apps 目录做了映射的，这个目录没有 jar 包，容器里面的 apps 目录也就没有 jar 包。所以我们可以把 jar 包拷贝到这个目录就好了。

![](http://cdn.passjava.cn/uPic/image-20230302170048653BDKnkz.png)

等等，为什么用 IDEA 启动容器时没有问题？因为当时没有 IDEA 做目录的映射。

启动完成后，我们来测试下部署的服务是否正常。

![](http://cdn.passjava.cn/uPic/image-20230302165826113NGWiZS.png)
