...
![img](http://cdn.passjava.cn/uPic/640-20220407092515374uVmfO9.png)

然后将占用最高的 pid 转换为 16 进制`printf '%x\n' pid`得到 nid

![img](http://cdn.passjava.cn/uPic/640-20220407092515406t7qCNQ.png)

接着直接在 jstack 中找到相应的堆栈信息`jstack pid |grep 'nid' -C5 –color`

![img](http://cdn.passjava.cn/uPic/640-20220407092515451QSAfpq.png)
...
![img](http://cdn.passjava.cn/uPic/640-20220407092515484XCezxN.png)
...
![img](http://cdn.passjava.cn/uPic/640-20220407092515517qB3gKh.png)

#### 上下文切换

针对频繁上下文问题，我们可以使用`vmstat`命令来进行查看

![img](http://cdn.passjava.cn/uPic/640-20220407092515552M21Bsq.png)

cs(context switch) 一列则代表了上下文切换的次数。如果我们希望对特定的 pid 进行监控那么可以使用`pidstat -w pid`命令，cswch 和 nvcswch 表示自愿及非自愿切换。

![img](http://cdn.passjava.cn/uPic/640-20220407092515584Wi5vj8.png)

### 磁盘

磁盘问题和 cpu 一样是属于比较基础的。首先是磁盘空间方面，我们直接使用`df -hl`来查看文件系统状态

![img](http://cdn.passjava.cn/uPic/640-20220407092515620RlRWvj.png)

更多时候，磁盘问题还是性能上的问题。我们可以通过 iostat,`iostat -d -k -x`来进行分析

![img](http://cdn.passjava.cn/uPic/640-20220407092515658BEbrVo.png)
...
![img](http://cdn.passjava.cn/uPic/640-20220407092515696evB1iM.png)

不过这边拿到的是 tid，我们要转换成 pid，可以通过 readlink 来找到 pid,`readlink -f /proc/*/task/tid/../..。`

![img](http://cdn.passjava.cn/uPic/640-202204070925157306ip4K4.png)

找到 pid 之后就可以看这个进程具体的读写情况`cat /proc/pid/io`

![img](http://cdn.passjava.cn/uPic/640-20220407092515761qNKjMz.png)

我们还可以通过 lsof 命令来确定具体的文件读写情况`lsof -p pid`

![img](http://cdn.passjava.cn/uPic/640-202204070925158283qSO1V.png)

### 内存

内存问题排查起来相对比 CPU 麻烦一些，场景也比较多。主要包括 OOM、GC 问题和堆外内存。一般来讲，我们会先用`free`命令先来检查一发内存的各种情况。

![img](http://cdn.passjava.cn/uPic/640-202204070925158620mb6BZ.png)
...
![img](http://cdn.passjava.cn/uPic/640-202204070925158932nIfqS.png)
...
![img](http://cdn.passjava.cn/uPic/640-20220407092515928oREkmA.png)
...
![img](http://cdn.passjava.cn/uPic/640-20220407092515976EARGo9.png)
...
![img](http://cdn.passjava.cn/uPic/640-202204070925160109ctMku.png)

或者直接通过查看`/proc/pid/task`的数量即为线程数量。

![img](http://cdn.passjava.cn/uPic/640-20220407092516039e6ndwj.png)
...
![img](http://cdn.passjava.cn/uPic/640-20220407092516079NAayxM.png)

我们如果确定有可疑的内存端，需要通过 gdb 来分析`gdb --batch --pid {pid} -ex "dump memory filename.dump {内存起始地址} {内存起始地址+内存块大小}"`

![img](http://cdn.passjava.cn/uPic/640-20220407092516108YTO2Ca.png)
...
![img](http://cdn.passjava.cn/uPic/640-20220407092516139eHaz4Y.png)

然后等放一段时间后再去看看内存增长的情况，通过`jcmd pid VM.native_memory detail.diff(summary.diff)`做一下 summary 或者 detail 级别的 diff。

![img](http://cdn.passjava.cn/uPic/640-20220407092516168NifEGG.png)

![img](http://cdn.passjava.cn/uPic/640-202204070925162309mn6Z9.png)
...
![img](http://cdn.passjava.cn/uPic/640-20220407092516263IrNGo9.png)

此外在系统层面，我们还可以使用 strace 命令来监控内存分配`strace -f -e "brk,mmap,munmap" -p pid`这边内存分配信息主要包括了 pid 和内存地址。

![img](http://cdn.passjava.cn/uPic/640-20220407092516307xQyqWD.png)
...
![img](http://cdn.passjava.cn/uPic/640-20220407092516452Gc4WZ2.png)
...
![img](http://cdn.passjava.cn/uPic/640-20220407092516561ZLvq0j.png)
...
![img](http://cdn.passjava.cn/uPic/640-20220407092516645jZbVKp.png)

如上图所示，overflowed 表示全连接队列溢出的次数，sockets dropped 表示半连接队列溢出的次数。

**「ss 命令，执行 ss -lnt」**

![img](http://cdn.passjava.cn/uPic/640-20220407092516708iV15hE.png)
...
![img](http://cdn.passjava.cn/uPic/640-20220407092516767U5lO1I.png)

接下来我们通过 wireshark 打开抓到的包，可能就能看到如下图所示，红色的就表示 RST 包了。

![img](http://cdn.passjava.cn/uPic/640-20220407092516829tgYSha.png)
...
![img](http://cdn.passjava.cn/uPic/640-20220407092516896pP4xzh.png)
