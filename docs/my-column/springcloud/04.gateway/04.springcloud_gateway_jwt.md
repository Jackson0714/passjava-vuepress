---
title: 手摸手 Spring Cloud Gateway + JWT 实现登录认证
date: 2024-08-18
---
...
![目录](http://cdn.passjava.cn/uPic/image-20220815090234699f67G4E.png)
...
![认证和校验身份流程](http://cdn.passjava.cn/uPic/image-202208142218310444AGSjdwqWPvG.png)
...
![PassJava-Platform 框架](http://cdn.passjava.cn/uPic/image-20220814114324471jr5WGX.png)

### 认证服务：passjava-auth

![passjava-auth 服务](http://cdn.passjava.cn/uPic/image-202208141143492007qKkh9.png)

核心类就是 JwtAuthController 类，里面有登录接口和刷新令牌的接口。

### 网关服务：passjava-gateway

![passjava-gateway 服务](http://cdn.passjava.cn/uPic/image-20220814114417058SCsk85.png)
...
![passjava-jwt 服务](http://cdn.passjava.cn/uPic/image-20220814114509406dR6UDZ.png)
...
![passjava-member 服务](http://cdn.passjava.cn/uPic/image-20220814114548560zCVFYl.png)
...
![](http://cdn.passjava.cn/uPic/image-20220814111451193gRCzqZ.png)

检查下 nacos 注册中心上是否注册了这三个服务：可以看到确实有上面的三个微服务。

![](http://cdn.passjava.cn/uPic/image-20220814111725968N0Qw2A.png)

## 如何做登录认证

登录认证就是校验下用户提交的账户名和密码与本地数据库中的是否完全匹配，如果匹配，就认证通过。就是下方这个流程的 1、2、3 步。

![](http://cdn.passjava.cn/uPic/image-20220814105416774WMesSj.png)
...
![image-20220814161920022](http://cdn.passjava.cn/uPic/image-20220814161920022TSi6Tf.png)
...
![](http://cdn.passjava.cn/uPic/image-20220814163415719HMyHcP.png)
...
![登录 API](http://cdn.passjava.cn/uPic/image-202208141655254572aPJrp.png)
...
![流程图-生成 JWT 令牌](http://cdn.passjava.cn/uPic/image-2022081417084513103ogTt.png)

生成令牌的核心代码如下：

![生成 JWT 的核心代码](http://cdn.passjava.cn/uPic/image-20220814172121051Rqwmfq.png)

使用这个工具类的前提是我们需要先引入 jjwt 依赖。这个在 passjava-jwt 项目的 pom 文件中引入。

![引入 jjwt 依赖](http://cdn.passjava.cn/uPic/image-20220814172217792z4J5WV.png)

用 Postman 工具调用后，可以看到生成的令牌如下：

![生成令牌](http://cdn.passjava.cn/uPic/image-20220814182655100H4H07N.png)

用 base64 解码后，可以看到 token 中的 PAYLOAD 里面包含了用户 id 和用户名。

![](http://cdn.passjava.cn/uPic/image-20220814171711910cic4Jn.png)

生成 JWT 的加密密钥一般都是写到配置文件中。这里我是配置在 passjava-jwt 项目的 application-jwt.yml 配置文件中的。

![JWT 配置项](http://cdn.passjava.cn/uPic/image-20220814181404573xWi2V2.png)

然后认证服务就会将 JWT 令牌返回给客户端了。当客户端想要查询这个 userId 对应的会员信息时，就可以在请求的 Header 中带上 JWT 令牌。

## 如何携带 JWT 发送请求

![](http://cdn.passjava.cn/uPic/image-20220814184517467cvInSy.png)
...
![](http://cdn.passjava.cn/uPic/image-20220814180402617UdX4Rz.png)

![](http://cdn.passjava.cn/uPic/image-20220814180431393sqNqcg.png)
...
![去掉 Bearer 前缀](http://cdn.passjava.cn/uPic/image-202208141825224276nl5ld.png)

## 网关如何验证 JWT 和转发请求

![网关验证 Token和转发请求](http://cdn.passjava.cn/uPic/image-20220814194440430RIKvF0.png)
...
![网关的全局过滤器 JwtAuthCheckFilter](http://cdn.passjava.cn/uPic/image-20220814185733422F5KMsJ.png)

## 会员服务处理业务逻辑

![](http://cdn.passjava.cn/uPic/image-20220814221220859aWnQGr.png)
...
![](http://cdn.passjava.cn/uPic/image-20220814195455216njzYqE.png)
...
![](http://cdn.passjava.cn/uPic/image-20220814195944424JfFsIz.png)

将拦截器注册到 WebMvcConfigurer。文件名为 WebMvcConfig.java。![](http://cdn.passjava.cn/uPic/image-20220814220241678N9udz3.png)
...
![](http://cdn.passjava.cn/uPic/image-20220814224556708tM1RKs.png)
...
![生成 access_token 和 refresh_token](http://cdn.passjava.cn/uPic/image-20220815085804556JqzMJA.png)
...
![缓存令牌](http://cdn.passjava.cn/uPic/image-20220815083736227NZOnFi.png)
